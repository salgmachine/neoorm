...
class ObjectGraphImpl implements ObjectGraph {
	...
	public Index<Node> getIndexService(boolean fulltext) {
		if (fulltext)
			return ineo.getFullTextIndexService();
		else
			return ineo.getIndexService();
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see jo4neo.ObjectGraph#getAddedBetween(java.lang.Class, java.util.Date,
	 * java.util.Date)
	 */
	public <T> Collection<T> getAddedBetween(Class<T> t, Date from, Date to) {
		return new LoadOperation<T>(t, ineo).within(from.getTime(),
				to.getTime());
	}
	...
	private boolean supportsRecency(Class<?> c) {
		return c.isAnnotationPresent(neo.class)
				&& c.getAnnotation(neo.class).recency();
	}
	 ...
	 *            the key
	 * @param value
	 *            the query string, (part of) the value
